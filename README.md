...
- **Режим «Закадровый» (voice-over)** — переведённый голос звучит **поверх приглушённого оригинала**; громкость оригинальной дорожки регулируется прямо в редакторе
- **Субтитры — только на языке оригинала** — режим субтитров кладёт на видео субтитры **без перевода**, оригинальная дорожка сохраняется, плей работает прямо в редакторе
- **Все режимы переключаются в редакторе** — уже загруженный ролик можно отправить в любой режим: Дубляж / Закадровый / Субтитры / Шуточный / Транскрипт (раньше часть была только на старте)
- **Превью выбранного видео** на главном экране — первый кадр прямо в зоне загрузки
- **Экспорт не пересинтезирует одобренную озвучку** — вручную перегенерированные фразы сохраняются как есть
- **Реальные альтернативные кванты Gemma** — Q5_K_M / Q6_K / Q8_0 с выбором варианта, точнее дефолтного Q4_0 (выбор реально применяется в рантайме)
- **Авто-обновление внутри приложения** — новая версия ставится по кнопке

<details><summary>Что было в 2.1 / 2.0</summary>

- **Режим «Транскрипт + диаризация»** — отдельный экран: чистый транскрипт с раскладкой по спикерам + создание голоса из каждого спикера в один клик, экспорт `.srt` / `.txt`
- **Голоса из спикеров чистятся и авто-транскрибируются** — реф прогоняется через вокал-сепарацию и получает реф-текст автоматически, как в Higgs → клон качественнее
- **Разные голоса на разных спикерах** в пак-режиме
- **Блюр-подложка ровно по размеру субтитра** — перевод целиком лежит на своей размытой подложке
- **Монтажный гейн + громкость** — усиление всей дорожки видно прямо на вейформе
- **Синхронизация таймлайна и транскрипта**, **пакетная обработка**, **любые видео-форматы**
...
- **Закадровый перевод (voice-over)** — как дубляж, но оригинальная дорожка **слышна приглушённой под переводом**; баланс регулируется ползунком в редакторе
- **Режим субтитров** — субтитры **на языке оригинала** без дубляжа и перевода, оригинальный звук сохраняется; для быстрой раздачи ролика с подписями
- **Пять режимов, переключаемых на лету** — Дубляж / Закадровый / Субтитры / Шуточный / Транскрипт прямо в редакторе на уже загруженном ролике
- **Диаризация спикеров** — кто и когда говорит (NVIDIA **Sortformer** v2, до 4 голосов), разный голос на каждого спикера
- **Режим «Транскрипт + диаризация»** — отдельный экран: аудио/видео → чистый транскрипт с раскладкой по спикерам (клик по фразе двигает плейхед и наоборот), из каждого спикера **одной кнопкой создаётся голос** — реф чистится вокал-сепарацией и авто-транскрибируется под реф-текст (как в Higgs). Экспорт `.srt` / `.txt`
...
- **Пакетная обработка** — очередь из нескольких файлов, каждый прогоняется одними настройками, прогресс по каждому
...
Главный экран — 5 режимов (дубляж / закадровый / субтитры / шуточный / транскрипт), превью выбранного видео, выбор языков, приём любых видео-форматов:

![Главный экран Dub Studio](docs/screenshot-home.png)

Живой редактор дубляжа — субтитры с подложкой, локализованный тайтл, голоса по спикерам, гейн, вейформа (скриншот выше).

Режим «Транскрипт» — диаризованный транскрипт с раскладкой по спикерам и созданием чистых голосов из каждого спикера в один клик:
