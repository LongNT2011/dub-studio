...
- **Режим «Транскрипт + диаризация»** — новый отдельный экран: чистый транскрипт с раскладкой по спикерам + создание голоса из каждого спикера в один клик, экспорт `.srt` / `.txt`
- **Голоса из спикеров чистятся и авто-транскрибируются** — реф прогоняется через вокал-сепарацию (без фона и музыки) и получает реф-текст автоматически, как в Higgs → клон качественнее
- **Разные голоса на разных спикерах** в пак-режиме — каждому диаризованному спикеру назначается свой голос
- **ref-текст в клоне дубляжа** — Higgs получает расшифровку референса (авто-транскрипция реф-клипа) → чище тембр
- **Блюр-подложка ровно по размеру субтитра** — оригинал спрятан, а перевод целиком лежит на своей размытой подложке (не половиной на чистом видео)
- **Монтажный гейн + громкость** — усиление всей дорожки видно прямо на вейформе; отдельный ползунок громкости прослушки
- **Синхронизация таймлайна и транскрипта** — клик по фразе двигает плейхед, перемотка подсвечивает и подкручивает активную фразу
- **Пакетная обработка** — очередь из нескольких файлов за один прогон
- **Любые видео-форматы** — MP4, MOV, MKV, WEBM, AVI и другие
...
Главный экран — 4 режима (дубляж / субтитры / шуточный / транскрипт), выбор языков, приём любых видео-форматов:
