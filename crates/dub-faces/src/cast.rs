...
///
/// Комбайнер = MAX(лицо, голос) + малый бонус за согласие, а НЕ линейное среднее. Причина: аватар-лицо
/// НЕНАДЁЖНО (кадр берётся из говорящих сегментов, но камера часто показывает слушателя, а не говорящего —
/// без active-speaker detection лицо бывает чужим, косинус ~шум). Голос же считается WeSpeaker'ом по РЕЧИ
/// самого спикера -> надёжно идентифицирует актёра. Линейное среднее 0.6·face+0.4·voice позволяло шумному
/// лицу ВЕТТИТЬ верный голосовой матч (0.6·0.05+0.4·0.5=0.23 < порога). MAX-комбайнер: сильного сигнала
/// ЛЮБОЙ модальности (тот же актёр по голосу ИЛИ по лицу) достаточно; разные люди дают ~0.05-0.2 по обеим,
/// т.е. ложных матчей почти нет. Клампим к 1.0.
pub fn character_similarity(a: &Character, b: &Character) -> f32 {
    ...
    match (face, voice) {
        (Some(f), Some(v)) => (f.max(v) + 0.1 * f.min(v).max(0.0)).min(1.0),
        (Some(f), None) => f,
        (None, Some(v)) => v,
        (None, None) => -1.0,
    }
